...
### Story 3.3: 物种树与基因树输出 ⏭️ Post-MVP

> **已推迟至 Post-MVP。** OrthoFinder 运行时已生成树文件（SpeciesTree_rooted.txt、Gene_Trees/），后续可按需提取格式化。
...
### Story 3.4: Copy Number 统计 ⏭️ Post-MVP

> **已推迟至 Post-MVP。** OrthoFinder 已输出 Orthogroups.GeneCount.tsv，后续可按需扩展统计分析。
...
用户可以基于 Liftoff 映射结果，通过三分类体系（present/uncertain/missing）区分真缺失与注释假缺失，获得可信的缺失候选清单和增强注释文件。
...
**Given** 参考物种注释（GFF3）、参考物种 FASTA 和目标物种 FASTA
**When** 执行 liftoff_map rule
**Then** LiftoffAdapter 调用 Liftoff 1.6.x 将参考注释映射到目标 assembly
**And** 输出到 `results/liftoff/{reference}_to_{target}/`：
- `lifted_annotation.gff3` — 成功映射的注释
- `unmapped_features.txt` — 未映射基因列表
- `liftoff_stats.tsv` — 映射统计

**Given** 需要批量处理多个比较对
**When** 执行 liftoff_all rule
**Then** 触发所有 reference-target 组合的映射，并通过 liftoff_summary 汇总统计

**实现细节：** LiftoffAdapter 继承 BaseAdapter，支持可配置参数（min_coverage、min_identity、copies、flank），超时保护（默认 60 分钟），SGE 集群集成，30+ 单元测试。
...
**Given** Liftoff 映射结果（lifted_annotation.gff3 + unmapped_features.txt）
**When** 执行 liftoff_classify rule，基于 coverage/identity 阈值分析映射状态
**Then** 将基因三分类为：
- **present**：coverage ≥ min_coverage AND identity ≥ min_identity（成功映射）
- **uncertain**：coverage > 0 但低于阈值（部分映射，可能的假缺失）
- **missing**：coverage = 0（未映射，真缺失候选）

**Given** 分类完成
**When** 生成输出文件
**Then** 输出到 `results/liftoff/{reference}_to_{target}/`：
- `gene_classification.tsv` — 全基因分类（gene_id, gene_name, reference_species, target_species, status, coverage, identity）
- `missing_genes.tsv` — missing + uncertain 基因子集
- `absence_summary.tsv` — 跨比较对汇总（present/uncertain/missing 计数和比例）

**实现细节：** 默认阈值 min_coverage=0.50、min_identity=0.50（可配置），支持 gzip 输入，26+ 单元测试覆盖边界条件和阈值参数化。
...
As a **计算生物学研究员**,
I want **生成包含 Liftoff 补全的增强注释文件**,
So that **我可以使用更完整的注释进行下游分析**。

**Acceptance Criteria:**

**Given** gene_classification.tsv 中的分类结果和 lifted_annotation.gff3
**When** 执行 liftoff_enhance rule
**Then** 输出 `lifted_annotation_enhanced.gff3`，为每个特征（gene、mRNA、CDS、exon）添加属性：
- `liftoff_coverage` — Liftoff 覆盖率
- `liftoff_identity` — Liftoff 一致性
- `liftoff_status` — 分类状态（present/uncertain/missing）
- `source_gene` — 参考基因 ID
- `reference_species` — 参考物种名

**Given** 默认过滤策略
**When** 生成增强 GFF3
**Then** 仅包含 status=present 的基因（可通过 include_uncertain 配置包含 uncertain 基因）

**Given** 补全注释与原始注释有冲突
**When** 合并时
**Then** 优先保留原始注释，Liftoff 结果作为补充；属性自动从 gene 级传播到子特征

**实现细节：** 原子写入（.tmp → rename），支持 gzip，20+ 单元测试。
